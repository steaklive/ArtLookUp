...
namespace ArtLookUp
{
    ...
    public class PinchToZoom : ContentView
	{
        ...
        public void OnPanUpdated(object sender, PanUpdatedEventArgs e)
        {
            ...
            switch (e.StatusType)
            {
                case GestureStatus.Running:
                    ...
                    bool canMoveX = width > App.Current.MainPage.Width;
                    bool canMoveY = height > App.Current.MainPage.Height;

                    // In the Below Code we r Pinching  the Image for the Particular Corner with MoveX and MoveY  interface    element bounds.

                    if (canMoveX)
                    {
                        double minX = (width - (App.Current.MainPage.Width / 2)) * -1;
                        double maxX = Math.Min(App.Current.MainPage.Width / 2, width / 2);
                    ...
                    if (canMoveY)
                    {
                        double minY = (height - (App.Current.MainPage.Height / 2)) * -1;
                        double maxY = Math.Min(App.Current.MainPage.Height / 2, height / 2);
